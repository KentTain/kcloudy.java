...
public enum AppLogType implements EnumBase {
	/**
	 * 应用日志
	 */
	Application("应用日志", 0),
	 ...
	 * 数据模型定义日志
	 */
	ModelDefinition("数据模型定义日志", 2),

	/**
	 * 关系模型定义日志
	 */
	RelationDefinition("关系模型定义日志", 3),
	...
	private AppLogType(String desc, Integer index) {
		this.desc = desc;
		this.index = index;
	}
	...
	public static AppLogType valueOf(Integer index) {
		for (AppLogType type : values()) {
	 ...
	 */
	public static java.util.Map<Integer, String> getList(List<AppLogType> exceptEnums) {
