...
public class WebApiSecurityConfig extends ResourceServerConfigurerAdapter {
    ...
    public void configure(HttpSecurity http) throws Exception {
    	...
    	SpringSecurityUtil.securityConfig(http, tenantResolver, securityFilter, idTokenResponseClient, clientRegistrationRepository);
    }
...
//				IdSrvAuthorizationRequestResolver.REGISTRATION_ID_URI_VARIABLE_NAME);
		String redirectUrl = String.format("{baseUrl}/login/oauth2/code/{%s}",
				IdSrvAuthorizationRequestResolver.REGISTRATION_ID_URI_VARIABLE_NAME);
        System.out.println(String.format("-----WebApiSecurityConfig idsvrClientRegistration: %s in sso domain: %s to redirect: %s" ,
		...
		return ClientRegistration.withRegistrationId(OpenIdConnectConstants.ClientAuthScheme)
				...
				.jwkSetUri(ssoWebDomain + OpenIdConnectConstants.OAuth2_JwkSet_Action)
				.build();
	}
}
