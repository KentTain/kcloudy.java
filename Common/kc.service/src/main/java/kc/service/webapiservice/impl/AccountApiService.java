...
public class AccountApiService extends IdSrvWebApiServiceBase implements IAccountApiService{
	...
	public ServiceResult<Boolean> ChangePassword(String userId, String currentPassword, String newPassword) {
		...
		result = WebSendGet(
            ...
            AccountApiUrl() + "AccountApi/ChangePasswordAsync?userId=" + userId + "&currentPassword=" + currentPassword + "&newPassword=" + newPassword,
	...
	public ServiceResult<Boolean> ChangeMailPhone(String userId, String email, String phone) {
		...
		result = WebSendGet(
            ...
            AccountApiUrl() + "AccountApi/ChangeMailPhoneAsync?userId=" + userId + "&email=" + email + "&phone=" + phone,
            ApplicationConstant.AccScope,
            callback ->
            {
            	return callback;
	...
	@Override
	public PaginatedBaseDTO<OrganizationSimpleDTO> LoadPaginatedOrganizationsWithUsersByFilter(RootOrgWithUsersPaginatedSearchDTO searchModel){
		String jsonData = SerializeHelper.ToJson(searchModel);
