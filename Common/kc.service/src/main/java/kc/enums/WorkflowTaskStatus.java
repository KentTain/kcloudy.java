...
public enum WorkflowTaskStatus implements EnumBase {
	/**
	 * 未处理
	 */
	UnProcess("未处理", 0),
	...
	private WorkflowTaskStatus(String desc, Integer index) {
		this.desc = desc;
		this.index = index;
	}
	...
	public static WorkflowTaskStatus valueOf(Integer index) {
		for (WorkflowTaskStatus type : values()) {
	...
	public static String getDesc(Integer index) {
		for (WorkflowTaskStatus c : values()) {
			if (c.getIndex() == index) {
				return c.getDesc();
			}
		}
	 ...
	 */
	public static java.util.Map<Integer, String> getList(List<WorkflowTaskStatus> exceptEnums) {
