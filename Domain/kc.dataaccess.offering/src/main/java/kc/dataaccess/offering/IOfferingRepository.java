...
import kc.model.offering.Offering;
...
public interface IOfferingRepository extends JpaRepository<Offering, Integer> , JpaSpecificationExecutor<Offering> {
    ...
    @Query("from Offering as o where o.offeringCode LIKE %:code%" +
            ...
            " and o.status = :status ")
    Page<Offering> findAllByFilter(Pageable pageable, @Param("code") String code, @Param("name") String name, @Param("status") Integer status);

    @Query("from Offering as o where o.offeringCode LIKE CONCAT('%',:code,'%')" +
            " and o.offeringName LIKE CONCAT('%',:name,'%')")
    Page<Offering> findAllByFilter(Pageable pageable, @Param("code") String code, @Param("name") String name);
