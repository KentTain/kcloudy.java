    ...
    DownloadBlobFormatter: function (value, rec, index) {
        ...
        if (isArray) {
            ...
            for (var i = 0; i < value.length; i++) {
                ...
                if (blob != null)
                    aList.push('<a target="_blank" href="' + blob.downloadFileUrl + '" download="' + blob.blobName + '">' + blob.blobName + '</a>');
            }

            return aList.join('<br/>');
        }
        return '<a target="_blank" href="' + value.downloadFileUrl + '" download="' + value.blobName + '">' + value.blobName + '</a>';
    },
    /**Blob对象格式化为：显示图片
     * @return {string}
     */
...
$.extend($.fn.validatebox.defaults.rules, {
    ...
    compareDate: {
        validator: function (value, param) {
            return CompareUtil.dateCompare($(param[0]).datebox("getValue"), value);
        },
    ...
    compareTime: {
        validator: function (value, param) {
            return CompareUtil.timeCompare($(param[0]).timespinner("getValue"), value);
        },
    ...
    compareDateTime: {
        validator: function (value, param) {
            return CompareUtil.dateTimeCompare($(param[0]).timespinner("getValue"), value);
        },
        message: "结束时间不能小于或等于开始时间"
    },
    // 验证是否包含空格和非法字符
...
$.extend($.fn.datagrid.defaults.editors, {
    datebox: {
        init: function (container, options) {
            let input = $('<input type="text">').appendTo(container);
